    ...
    function __construct($imgsource, $sourcetype = QrReader::SOURCE_TYPE_FILE, $isUseImagickIfAvailable = true)
    {
        ...
        try {
            ...
            if($isUseImagickIfAvailable && extension_loaded('imagick')) {
                ...
                $source = new \Pctco\Coding\QRCode\Reader\IMagickLuminanceSource($im, $width, $height);
            }else {
                $width = imagesx($im);
                $height = imagesy($im);
                $source = new \Pctco\Coding\QRCode\Reader\GDLuminanceSource($im, $width, $height);
            }
            $histo = new \Pctco\Coding\QRCode\Reader\Common\HybridBinarizer($source);
            $bitmap = new \Pctco\Coding\QRCode\Reader\BinaryBitmap($histo);
            $reader = new \Pctco\Coding\QRCode\Reader\Qrcode\QRCodeReader();

            $this->result = $reader->decode($bitmap);
        }catch (\Pctco\Coding\QRCode\Reader\NotFoundException $er){
            $this->result = false;
        }catch( \Pctco\Coding\QRCode\Reader\FormatException $er){
            $this->result = false;
        }catch( \Pctco\Coding\QRCode\Reader\ChecksumException $er){
            $this->result = false;
        }
    }
