...
class Ed{
   ...
   * @name Encrypt
   * @describe 加密
   * @param mixed $string 需要加密的字符串
   * @return String
   **/
   public function en($string) {
      $mt_rand = $this->config['key'] === true?mt_rand(0,0):mt_rand(0,9999);
      $step1 = array();
      for ($i = 0; $i < strlen($string); $i++) {
         if (in_array($string[$i], $this->ref_num)) {
            ...
            array_push($step1, base64_encode("<".$mt_rand.">"));
         }
         else {
      ...
      $string = "MC0w";
      $delimiter = $mt_rand.$string.$mt_rand;
   ...
   * @name Decrypt
   * @describe 解密
   * @param mixed $string 需要解密的字符串
   * @return String
   **/
   public function de($hash) {
